<template>
    <div class="field">
        <fieldset class="radio field__fieldset">
            <legend v-if="label" class="field__title field-title">{{ label }}</legend>
            <label v-for="option in options" :key="option.value" class="radio__item field__radio">
                <input type="radio" class="radio__input" :name="name" :value="option.value"
                    :checked="value === option.value" @change="handleChange(option.value)" />
